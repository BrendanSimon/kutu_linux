...
u32 GR1000_Status(struct gr1000_drvdata *gr1000)
{
   ...
   status = gr1000_read_reg(gr1000, R_RUN_STATUS);
   status &= 0x000000ff;
...
int GR1000_Set_Pulse(struct gr1000_drvdata *gr1000, void *user_ptr)
{
   ...
   struct GR1000_pulse_struct pulse;
   ...
   if (copy_from_user(&pulse, user_ptr, sizeof(pulse))) {
      printk(KERN_DEBUG "GR1000_Set_Pulse: copy failed\n");

      return -EFAULT;
   }

   if (GR1000_Status(gr1000) & SWEEP_RUNNING_FLAG)
      return -2;

   if (GR1000_Status(gr1000) & TEST_RUNNING_FLAG)
      return -3;
   ...
   gr1000_write_reg(gr1000, R_PULSE_RATE, rate_reg);
   gr1000_write_reg(gr1000, R_PULSE_WIDTH, width_reg);
...
int GR1000_Run_Test(struct gr1000_drvdata *gr1000, void *user_ptr)
{
   ...
   if (copy_from_user(&cmd, user_ptr, sizeof(cmd))) {
      printk(KERN_DEBUG "GR1000_Run_Test: copy failed\n");

      return -EFAULT;
   }

   if (GR1000_Status(gr1000) & SPI_BUSY_FLAG)
      return -1;
   ...
   gr1000_write_reg(gr1000, R_BOTDA_START_FREQ, cmd.botda_start_freq);
   gr1000_write_reg(gr1000, R_BOTDA_END_FREQ, cmd.botda_end_freq);

   gr1000_write_reg(gr1000, R_ADC_COUNT, adc_count_value);
   gr1000_write_reg(gr1000, R_BOTDA_COUNT, cmd.botda_count);
   gr1000_write_reg(gr1000, R_MEM_STRIDE, cmd.row_stride);

   cmd.config &= (MODE_BITS|TEST_DATA_ENABLE|DAC_UPDATE_ENABLE);
   gr1000->config_state &= ~(MODE_BITS|TEST_DATA_ENABLE|DAC_UPDATE_ENABLE);
   gr1000->config_state |= cmd.config;
   gr1000_write_reg(gr1000, R_CONFIG, gr1000->config_state);

   if (cmd.run_test != RUN_NO_TEST)
      gr1000_write_reg(gr1000, R_RUN_TEST, START_TEST);
...
{
   struct GR1000_continuous_cmd_struct cmd;
   struct GR1000_read_data_struct *read_cmd;

   u32   adc_count_value,dma_size,mode;
   u32   status;

   printk(KERN_DEBUG "ioctl: entered GR1000_Continuous scan\n");

   if (copy_from_user(&cmd, user_ptr, sizeof(cmd))) {
      printk(KERN_DEBUG "GR1000_Continuous_Scan: copy failed\n");
   ...
   status = GR1000_Status(gr1000);
   printk(KERN_DEBUG "ioctl: GR1000_Continuous_scan, status = 0x%x\n",status);
   ...
   gr1000->config_state = DMA_ACTIVE|CLK_200_MODE|cmd.config;
   ...
   gr1000_write_reg(gr1000, R_BOTDA_START_FREQ, 0);
   ...
   gr1000_write_reg(gr1000, R_BOTDA_COUNT, 1);
   ...
   gr1000_write_reg(gr1000, R_DMA_SIZE, dma_size);
#ifdef DEBUG
...
int GR1000_Read_Data(struct gr1000_drvdata *gr1000, struct GR1000_read_data_struct *cmd)
{
   ...
   gr1000dma_write_reg(gr1000, S2MM_DMACR, 4);
   ...
   // set DMA mode
   if (cmd->dma_active == 1)
      gr1000->config_state |= DMA_ACTIVE;
   else
      gr1000->config_state &= ~DMA_ACTIVE;
   ...
   gr1000_write_reg(gr1000, R_DATA_READ_ADDR, (u32)(addr & 0x00000000ffffffff));
   gr1000_write_reg(gr1000, R_DATA_READ_STRIDE, row_stride);
   gr1000_write_reg(gr1000, R_DATA_READ_COL_COUNT, (num_columns*data_size/2));
   gr1000_write_reg(gr1000, R_DATA_READ_ROW_COUNT, num_rows);
//   gr1000_write_reg(gr1000, R_DATA_READ_START, size);

   // if DMA transfer then start controller
   if (cmd->dma_active == 1) {
      // start write to memory (max = 8M - 1), so use 4Mbyte blocks max
#ifdef DEBUG
      printk(KERN_DEBUG "S2MM_DA = 0x%x, S2MM_LENGTH = 0x%x = %d\n",(gr1000->dma_handle + cmd->dma_addr_offset),(size*8),(size*8));
#endif

      gr1000dma_write_reg(gr1000, S2MM_DMACR, 1);
      gr1000dma_write_reg(gr1000, S2MM_DA, gr1000->dma_handle + cmd->dma_addr_offset);
      gr1000dma_write_reg(gr1000, S2MM_LENGTH, (size*8));
      gr1000_write_reg(gr1000, R_DATA_READ_START, size);
   } else {
      gr1000_write_reg(gr1000, R_DATA_READ_START, size);
      ...
      //
      i = 0;
      while ((GR1000_Status(gr1000) & FIFO_EMPTY_FLAG) && (i < MAX_WAIT_COUNT)) {
         i++;
         if ((i&31) == 31) {
#ifdef DEBUG
...
int GR1000_Read_Frequency(struct gr1000_drvdata *gr1000, void *arg_ptr)
{
   ...
   for(i = 0; i<4096; i+=4)
      tmp[i] = readl(gr1000->base + R_GR1000_SWEEP_RAM_BASE + i)&0x000FFFFF;
...
int GR1000_SPI_Write(struct gr1000_drvdata *gr1000, void *user_ptr)
{
   ...
   for (j = 0; j < cmd.num_spi_writes; j++) {
      ...
      //
      i = 0;
      while ((GR1000_Status(gr1000) & SPI_BUSY_FLAG) && (i < MAX_WAIT_COUNT))
         i++;
#ifdef DEBUG
      ...
      gr1000_write_reg(gr1000, addr, cmd.port_data[j]);
...
//
int GR1000_Run_Sweep(struct gr1000_drvdata *gr1000, void *user_ptr)
{
   u32 i,*tmp,num_sweeps;
   struct GR1000_sweep_cmd_struct cmd;

   if (copy_from_user(&cmd, user_ptr, sizeof(cmd))) {
      printk(KERN_DEBUG "GR1000_Run_Sweep: copy failed\n");
   ...
   if (num_sweeps == 1024)
      gr1000->config_state &= ~DATA_SWEEP;
   else
      gr1000->config_state |= DATA_SWEEP;

   // start sweep test
   gr1000_write_reg(gr1000, R_CONFIG, gr1000->config_state);
   gr1000_write_reg(gr1000, R_RUN_TEST, SWEEP_START);

   //
   // Wait until sweep is finished, allow 20 seconds
   //
   i = 0;
   while ((GR1000_Status(gr1000) & SWEEP_RUNNING_FLAG) && (i < 1000)) {
   ...
   for (i=0; i<num_sweeps; i++) {
      tmp[i] = readl(gr1000->base + R_GR1000_SWEEP_RAM_BASE + i*4);
   }
...
int GR1000_Read_DAC_Table(struct gr1000_drvdata *gr1000, u32 *read_data)
{
   ...
   for(i = 0; i<1024; i++) {
      dac_table[i] = readl(gr1000->base + R_GR1000_DAC_LOOKUP_BASE + i*4)&0x000FFFFF;
   }
...
int GR1000_Write_DAC_Table(struct gr1000_drvdata *gr1000, u32 *write_data)
{
   ...
   for(i = 0; i<1024; i++) {
      // 18 or 20 bit DAC
      gr1000_write_reg(gr1000, (R_GR1000_DAC_LOOKUP_BASE + i*4), 0x00100000|(dac_table[i]&0x000FFFFF));

      test = readl(gr1000->base + R_GR1000_DAC_LOOKUP_BASE + i*4);
