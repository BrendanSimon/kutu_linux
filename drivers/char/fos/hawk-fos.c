...
#define DRIVER_NAME "fos"
//#define DRIVER_NAME "fos_pdrv"
#define MODULE_NAME "fos"
 ...
 **/
static irqreturn_t fos_isr(int irq, void *data)
{
...
static int fos_probe(struct platform_device *pdev)
{
   ...
   id = of_match_node(fos_of_match_table, pdev->dev.of_node);
   if (!id)
      return -EINVAL;
   ...
   dev_info(&pdev->dev, "Kutu FOS trying to call platform get resource\n");

   mem = platform_get_resource(pdev, IORESOURCE_MEM, 0);
   fos->base = devm_ioremap_resource(&pdev->dev, mem);
   fos->dma_base = fos->base + 0x10000;

   if (IS_ERR(fos->base))
      return PTR_ERR(fos->base);

   fos->config_state = 0;

   dev_info(&pdev->dev, "Kutu FOS finished call to platform get resource\n");

   dev_info(&pdev->dev, "Kutu FOS trying to allocate irq\n");

   fos->irq = platform_get_irq(pdev, 0);
   ret = devm_request_irq(&pdev->dev, fos->irq, &fos_isr, 0, dev_name(&pdev->dev), fos);
   if (ret) {
      dev_err(&pdev->dev, "No IRQ available");
      return ret;
   }

   dev_info(&pdev->dev, "Kutu FOS successfully setup irq\n");

   platform_set_drvdata(pdev, fos);
   ...
   dev_info(&pdev->dev, "Kutu FOS finished loading driver\n");
...
static int fos_remove(struct platform_device *pdev)
{
   struct fos_drvdata *fos;

   fos = platform_get_drvdata(pdev);
...
MODULE_DESCRIPTION("Hawk FOS Linux driver");
